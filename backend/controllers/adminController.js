...
const addDoctor = async (req, res) => {
  try {
    ...
    if (!name || !name.trim()) {
      return res.json({ success: false, message: "Doctor name is required" });
    }
    if (!email || !email.trim()) {
    ...
    if (!degree || !degree.trim()) {
      return res.json({ success: false, message: "Degree/Education is required" });
    }
    if (!experience || !experience.trim()) {
      return res.json({ success: false, message: "Experience is required" });
    }
    if (!about || !about.trim()) {
      return res.json({ success: false, message: "About doctor is required" });
    }
    if (!fees || fees <= 0) {
      return res.json({ success: false, message: "Valid fees amount is required" });
    }
